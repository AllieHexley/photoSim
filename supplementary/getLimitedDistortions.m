...
function [display] = getLimitedDistortions(matchedSignals,Sim,display,displayPrimaries,T_cies026,mb026,smallestBit,nPrimaries,bits);

% add in here what happens if limited to an N bit display
...
for i = 1:length(Sim.ss)
    if nPrimaries<=5
        ...
        % now round to what RGB is actually allowed
        bitNum = 2^bits;
        updown{i} = round(rgbSetting{i} * bitNum)/bitNum;
        rgbRealized{i} = (updown{i}'.*displayPrimaries); 
        ...
        bitNum = 2^bits;
        updown{i} = round(rgbSetting{i} * bitNum)/bitNum;
        rgbRealized{i} = (updown{i}'.*displayPrimaries(:,[1,2,3,5,6])); 
    end
    if nPrimaries == 3
