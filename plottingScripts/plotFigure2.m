...
% set up colors
map=colormap(colorcet('rainbow'));
map2=colormap(colorcet('bjy'));
...
for i=1:2:13 % mark on crosses on daylight locus at every other CCT
    plot(DLmb(2,i),DLmb(1,i),'.','Color',map2(round(256*i/13),:),'LineWidth',2,'MarkerSize',50);
end
xlabel('L/(L+M)');
ylabel('S/(L+M)');
%yticklabels({});
...
print(fig, '../plots/fig2a_revised_insert.pdf','-dpdf');
...
for i=1:2:13 % mark crosses on daylight locus
    plot(DLmb(2,i),DLmb(3,i),'.','Color',map2(round(256*i/13),:),'LineWidth',2,'MarkerSize',50);
end
xlabel('L/(L+M)');
ylabel('I/(L+M)');
%yticklabels({});
...
fig.PaperPositionMode = 'manual';
fig.PaperPosition=[0.1 0.1 3 3];
print(fig, '../plots/fig2b_revised_insert.pdf','-dpdf');
...
for i=1:2:13 % plot crosses on daylight locus
    plot(DLmb(3,i),DLmb(1,i),'.','Color',map2(round(256*i/13),:),'LineWidth',2,'MarkerSize',50);
end
xlabel('I/(L+M)');
ylabel('S/(L+M)');
%yticklabels({});
...
fig.PaperPositionMode = 'manual';
fig.PaperPosition=[0.1 0.1 3 3];
print(fig, '../plots/fig2c_revised_insert.pdf','-dpdf');
